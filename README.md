...
bash scripts/import_openssf_scorecard_report.sh \
  ...
  --scope "../digg-scorecard-monitor/reporting/scope.json" \
  --output "data/reports/openssf_scorecard.json"
```

Samma JSON används av Hugo och av den schemalagda importen i GitHub Actions.
Alla rapporterade kodförråd för organisationen importeras, förutom de som ligger i `excluded` i `digg-scorecard-monitor/reporting/scope.json`.

Vid manuell körning importerar workflowen `Import OpenSSF Scorecard report` den monitor-rapport och scope som ligger i `diggsweden/digg-scorecard-monitor` vid körningstillfället.
