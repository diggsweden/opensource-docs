...
usage() {
  cat <<'EOF'
Usage: import_reuse_report.sh --report PATH --output PATH [--source-commit SHA] [--imported-at TIMESTAMP]

Imports the structured REUSE API JSON report as Hugo data.
Requires jq.
EOF
}
...
validate_report() {
  ...
  ' "$REPORT" >/dev/null || {
    echo "Invalid REUSE JSON report: $REPORT" >&2
    exit 1
  }
}
...
main() {
  ...
  echo "Imported REUSE report from $REPORT to $OUTPUT"
}
